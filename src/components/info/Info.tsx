...
const Info = () => {
  ...
  return (
    <div id='info'>
      ...
      >
        <p>POWsim is a proof of work simulator, but what is proof of work?</p>
        <p>Proof of work is most notably used in the Bitcoin network as trustless consensus method, meaning it serves as a means of verifying transactions without the need for a central authority such as a bank ....but how? The answers are in cryptography.</p>
        <p>At its core, proof of work is exactly how it sounds; it is proof that computational work (processing) was expended. But how do we expend this work and how do we prove that we did it?</p>
        <p>When you input a string into a cryptographic hash function, it returns a binary number (i.e. 01011100) with a fixed length, no matter the input size. This number is completely random; barely differing inputs will produce vastly different outputs, so there is no way to determine the input from the output. This implies that to find a hash output with desired attributes (for example, you want it to start with 10 zeroes), there is no better method than just guessing and checking. This is what the "work" in proof of work refers to.</p>
        <p>"Work" consists of repeatedly giving a cryptographic hash function a message combined with an incrementing (or randomized) arbitrary value until the output hash returned satisfies given requirements. Once that hash has been found, the aribitrary value that was combined with the message to create said hash IS THE PROOF that you did this work.</p>
